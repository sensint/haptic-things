...
// WiFi credentials
const char* ssid = "TU VEUX MON WIFI?"; // wifi credentials here (setup a hotspot with these or change it accordingly to ur hotspot)
const char* password = "ouiouioui";

// Web server on port 80
ESP8266WebServer server(80);

IPAddress local_IP(192, 168, 137, 104); // << Write your desired Static IP address here
...
int currentMode = 1; // 0 = Off, 1 = FSR, 2 = Gyro, 3 = Accel
int currentAxis = 0; // 0 = xyz, 1 = x, 2 = y, 3 = z, 4 = xy, 5 = yz, 6 = xz
int currentFreq = 150;
...
void handleRoot() {
  ...
  html += "<h2>Cube Portal #4</h2>";  // << Write ID of cube here (matching IP eg. xxx.xxx.xxx.104 = #4)
  ...
  html += "<div class='control'>";
  html += "<h3>Axis:</h3>";
  html += "<button id='axis0' onclick='setAxis(0)'>XYZ</button>";
  html += "<button id='axis1' onclick='setAxis(1)'>X</button>";
  html += "<button id='axis2' onclick='setAxis(2)'>Y</button>";
  html += "<button id='axis3' onclick='setAxis(3)'>Z</button>";
  html += "<button id='axis4' onclick='setAxis(4)'>XY</button>";
  html += "<button id='axis5' onclick='setAxis(5)'>YZ</button>";
  html += "<button id='axis6' onclick='setAxis(6)'>XZ</button>";
  html += "</div>";
  ...
  html += "function setAxis(axis) {";
  html += "  document.querySelectorAll('button[id^=\"axis\"]').forEach(btn => btn.classList.remove('active'));";
  html += "  document.getElementById('axis'+axis).classList.add('active');";
  html += "  fetch('/set?axis='+axis);";
  html += "}";
  ...
  html += "document.getElementById('axis" + String(currentAxis) + "').classList.add('active');";
...
void handleSet() {
  ...
  // Check if axis mode was changed
  if (server.hasArg("axis")) {
  currentAxis = server.arg("axis").toInt();
  changed = true;
  }
