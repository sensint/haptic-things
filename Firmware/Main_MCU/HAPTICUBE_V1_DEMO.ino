...
// Frequency sweetspots = 70 / 100 / 150 / 200 / 300
...
float deltaTresh = 20.0;  // threshohold imu = 100,  fsr = ~10/30 
float ampValue = 0.0;
...
String inputString;
int input0 = 0, input1 = 0, input3 = 0;
float input2;
...
void loop() {
  ...
  if (modeValue == 1) {
    ...
    // --- Group FSR --- //
    float groupA = abs((linSensor0 + linSensor5) /2 - offset); // Top/Bot      || A0/A5
    float groupB = abs((linSensor2 + linSensor4) /2 - offset); // East/West    || A2/A4
    float groupC = abs((linSensor3 + linSensor1) /2 - offset); // North/South  || A1/A3
    ...
    delay(10); // smoll delay to limit jitter in FSR
  }
...
void processCommand(char* cmd) {
  ...
  // Check we got exactly 4 parameters
  if (i != 4) {
    Serial1.println("ERR");
    return;
  }
  ...
  freqValue = atoi(tokens[1]);
  newAmpValue = atof(tokens[2]);
  if (newAmpValue < 0 || newAmpValue > 0.7) newAmpValue = 0.7;
  deltaTresh = atoi(tokens[3]);
